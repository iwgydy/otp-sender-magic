...
import { Terminal, Wifi, Shield, Zap, History } from "lucide-react";
...
const ConsoleOutput = ({ logs }: ConsoleOutputProps) => {
  ...
  const uniquePhones = new Set(logs.map(log => log.phone));
      ...
      {lastPhone && (
        <div className="mb-4 p-2 bg-gray-800/50 rounded-md border border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400">เบอร์ที่กำลังยิง: </span>
            <span className="text-sm text-green-400 font-mono">{lastPhone}</span>
            <span className="text-xs text-gray-500 ml-auto">
              ประวัติทั้งหมด: {uniquePhones.size} เบอร์
            </span>
          </div>
        </div>
      )}
      <ScrollArea className="h-[400px] rounded-md border border-gray-700 bg-black/60 p-4">
